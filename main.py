# Função para buscar linhas relevantes do CSV
def retrieve_relevant_rows(question, top_n=15):
    q = (question or "").lower()
    tokens = [t for t in set(q.split()) if len(t) > 2]
    if not tokens:
    ...
    if matched.empty:
        return df.head(70).to_string(index=False)
    return matched.head(top_n).to_string(index=False)
import os
import logging
import pandas as pd
import requests
...
# SYSTEM_PROMPT removido para evitar duplicidade
...
async def chat(message: Message):
    ...
    contexto_relevante = retrieve_relevant_rows(message.text, top_n=15)
    ...
    for model in models_to_try:
        try:
            payload = {
                "model": model,
                "messages": [
                    {"role": "system", "content": contexto_relevante},
    ...
    raise HTTPException(status_code=502, detail=f"Erro ao conectar com o serviço de IA: {last_error}")

@app.get("/api/health")
async def health():
    return {"status": "ok", "csv_loaded": len(df) > 0}
...
try:
    ...
    try:
        ...
        csv_context = df.head(70).to_string(index=False)
        # Truncar a string se ainda for muito grande
        if len(csv_context) > 150000:
            csv_context = csv_context[:150000] + "\n... (truncated)"
    except Exception:
...
- Se a data for mais que atual use "Proximo"
- Se a data for menor que atual user "Passado" 
